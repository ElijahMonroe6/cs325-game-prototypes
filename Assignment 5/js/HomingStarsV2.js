...
var HomingStarsV2 = {
    ...
    preload: function () {
		game.load.spritesheet('speedSheet', './assets/speedSheet.png', 37, 35);
    ...
    create: function () {
		...
		speed = game.add.sprite(game.world.randomX, game.world.randomY, 'speedSheet');
    ...
    bark: function () {
        if (game.time.now > nextBark && barks.countDead() > 0) {
            ...
            bork.play();
            bork._sound.playbackRate.value = Math.floor(Math.random() * (2 - 1) + 1) / 1;
    ...
    update: function () {
        ...
        enemies.forEach(function (item) {
            ...
            if( Math.sqrt(((speed.x - item.x) * (speed.x - item.x)) + ((speed.y - item.y) * (speed.y - item.y))) <= 275 ){
		...
		if (timer <= 0) {
            ...
            if (cursors.left.isDown || wasd.left.isDown) {
				if( Math.sqrt(((speed.x - player1.x) * (speed.x - player1.x)) + ((speed.y - player1.y) * (speed.y - player1.y))) <= 275 ){
        ...
        } else if (timer > 0) {
            {
                ...
                if (cursors.left.isDown || wasd.left.isDown) {
                    if( Math.sqrt(((speed.x - player1Ghost.x) * (speed.x - player1Ghost.x)) + ((speed.y - player1Ghost.y) * (speed.y - player1Ghost.y))) <= 275 ){
			
						if(speedScore >= 0){
					
							player1Ghost.body.velocity.x = -500;
					
						}
						else{
					
							player1Ghost.body.velocity.x = -300;
                ...
                if (cursors.up.isDown || wasd.up.isDown) {
					if( Math.sqrt(((speed.x - player1Ghost.x) * (speed.x - player1Ghost.x)) + ((speed.y - player1Ghost.y) * (speed.y - player1Ghost.y))) <= 275 ){
						...
						if(speedScore >= 0){
							...
							player1Ghost.body.velocity.y = -500;
						
						}
						else{
						
							player1Ghost.body.velocity.y = -300;
    ...
    ghostOverlapHandler: function () {
		...
		speed.x = game.world.randomX;
		speed.y = game.world.randomY;
		speedScore = 0;
